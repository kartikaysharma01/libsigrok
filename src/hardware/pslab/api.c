					  ...
					  const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if(!cg) {
		...
		case SR_CONF_OUTPUT_FREQUENCY:
			sr_err("GOAT: get freq ln 241, %f", devc->frequency);
		...
		case SR_CONF_DUTY_CYCLE:
			if (ch->type == SR_CHANNEL_LOGIC &&
				...
				*data = g_variant_new_double(docgp->duty_cycle * 100);
				sr_err("GOAT: get duty cycle ln 261, = %f", docgp->duty_cycle);
				break;
			}
			return SR_ERR_ARG;
		case SR_CONF_PHASE:
			if (ch->type == SR_CHANNEL_LOGIC &&
				...
				*data = g_variant_new_double(docgp->phase * 360);
				sr_err("GOAT: get phase ln 261, = %f", docgp->phase);
				break;
			}
			return SR_ERR_ARG;
		default:
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		...
		case SR_CONF_TRIGGER_LEVEL:
			...
			break;
		case SR_CONF_OUTPUT_FREQUENCY:
			sr_err("GOAT: here to set freq ln 310, = %f", g_variant_get_double(data));
			devc->frequency = g_variant_get_double(data);
			sr_err("GOAT: set freq ln 312, = %f", devc->frequency);
			break;
		default:
			return SR_ERR_NA;
		}
		...
		case SR_CONF_VDIV:
			...
			break;
		case SR_CONF_DUTY_CYCLE:
			sr_err("GOAT: here to set duty cycle ln 332, = %f", g_variant_get_double(data));
			if (ch->type == SR_CHANNEL_LOGIC &&
			    		ch->index < NUM_DIGITAL_OUTPUT_CHANNEL + NUM_ANALOG_CHANNELS) {
				docgp = cg->priv;
				...
				sr_err("GOAT: set duty cycle ln 337, = %f", docgp->duty_cycle);
				break;
			}
			return SR_ERR_ARG;
		case SR_CONF_PHASE:
			sr_err("GOAT: here to set phase ln 342, = %f", g_variant_get_double(data));
			if (ch->type == SR_CHANNEL_LOGIC && g_strcmp0(cg->name, "SQ1") &&
				...
				tmp = tmp / 360;
				docgp->phase = tmp;
				sr_err("GOAT: set phase ln 347, = %f", docgp->phase);
				break;
			}
			return SR_ERR_ARG;
		default:
